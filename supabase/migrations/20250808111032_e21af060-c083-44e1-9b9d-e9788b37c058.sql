-- Fix the ambiguous service_id reference
DO $$
DECLARE
  overijssel_region_id UUID;
  kunststof_service_id UUID;
BEGIN
  ...
  SELECT id INTO kunststof_service_id FROM services WHERE slug = 'kunststof-kozijnen';
  
  -- Create service if it doesn't exist
  IF kunststof_service_id IS NULL THEN
    ...
    RETURNING id INTO kunststof_service_id;
  ELSE
  ...
  AND NOT EXISTS (SELECT 1 FROM city_services cs WHERE cs.city_id = c.id AND cs.service_id = kunststof_service_id);
END $$;